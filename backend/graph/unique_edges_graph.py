...
class UniqueEdgesMetroGraph:
    ...
    def build_graph(self):
        ...
        for stop1, stop2, route_id in unique_edges_set:
            ...
            co2_emission = (distance / 1000) * 3.8
            # Ajouter l'arête
        ...
        logger.info(
            f"Nombre moyen de voisins par nœud: {sum(len(adj) for adj in self.adjacency.values()) / len(self.adjacency) if self.adjacency else 0:.2f}")
    ...
    def get_co2_between_stations(self, stop1: str, stop2: str) -> float:
        """
        Retourne la quantité totale de CO2 estimée entre deux stations, en grammes.
        Prend en compte toutes les lignes existantes entre stop1 et stop2.
        Retourne 0.0 si aucune connexion.
        """
        total_co2 = 0.0
        found = False

        for (s1, s2, route_id), edge_data in self.unique_edges.items():
            if {s1, s2} == {stop1, stop2}:  # Connexion dans les deux sens
                co2 = edge_data.get("co2", 0.0)
                total_co2 += co2
                found = True

        if not found:
            # Optionnel : logger l'absence de connexion
            logger.debug(f"Aucune arête trouvée entre {stop1} et {stop2} pour le calcul de CO2")
