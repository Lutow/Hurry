...
def build_optimized_database():
    ...
    print("🚀 CONSTRUCTION OPTIMISÉE DE LA BASE DE DONNÉES")
    print("=" * 60)
    ...
    try:
        ...
        for _, trip in trips.iterrows():
            ...
            if processed_count % 5000 == 0:
                print(f"📈 Traité {processed_count}/{len(trips)} trips...")
        edges_data = list(edges_set)
        print(f"✅ {len(edges_data)} arêtes préparées entre stations uniques")
        ...
        for i in range(0, len(edges_data), batch_size):
            ...
            if i % (batch_size * 5) == 0:
                print(f"📈 Inséré {i + len(batch)}/{len(edges_data)} arêtes...")
        ...
        print(f"   - {edges_count:,} arêtes métro")
...
def test_connectivity():
    ...
    """
    print("\n🔍 TEST DE CONNEXITÉ AVEC LA NOUVELLE BASE")
    print("=" * 60)
    ...
    try:
        ...
        # Tester la connexité
        print("🔍 Vérification de la connexité...")
        is_connected = graph_manager.connected()
        
        print(f"📊 Résultat: {'✅ RÉSEAU CONNEXE' if is_connected else '❌ RÉSEAU NON CONNEXE'}")
        ...
        if is_connected:
            ...
            if details['isolated_nodes']:
                print(f"🏝️ {len(details['isolated_nodes'])} stations isolées")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("❌ Échec de la construction de la base")
